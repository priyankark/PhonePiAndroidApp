...
toggleMagnetometer(value,url)
{
//const ws = new WebSocket('ws://192.168.1.102:5000//accelerometer');
const ws = new WebSocket('ws://'+url+'//magnetometer');
...
alert(e.message); };
  ...
  if(value===true)
  {
    ...
    SensorManager.startMagnetometer(this.state.time);
...
<Switch onValueChange={(value) => this.toggleMagnetometer(value,this.props.url)}
style={{marginBottom: 10}}
value={this.state.toggleSwitch}
style={{flex:0.25,alignItems:'center'}}
 />

<Subtitle style={{flex:0.5, alignItems:'center'}}>Magnetometer</Subtitle>
<TextInput
  placeholder={'Time'}
