...
toggleProximity(value,url)
{
//const ws = new WebSocket('ws://192.168.1.102:5000//accelerometer');
const ws = new WebSocket('ws://'+url+'//proximity');
...
alert(e.message); };
  ...
  if(value===true)
  {
    ...
    SensorManager.startProximity(this.state.time);
  ...
  **/
  ...
  ws.send(data.isNear.toString()+','+data.value.toString()+','+data.maxRange.toString());
...
<Switch onValueChange={(value) => this.toggleProximity(value,this.props.url)}
style={{marginBottom: 10}}
value={this.state.toggleSwitch}
style={{flex:0.25,alignItems:'center'}}
 />

<Subtitle style={{flex:0.5, alignItems:'center'}}>Proximity</Subtitle>
<TextInput
  placeholder={'Time'}
